...
install_podman_static() {
  ...
  download_if_missing "${PODMAN_STATIC_URL}" "${tarball}"
...
install_parallax() {
  local version="$1"
  local prefix="${CACHE_DIR}/prefix/parallax-${version}"

  if [ -x "${prefix}/bin/parallax" ] && [ -x "${prefix}/bin/parallax-mount-program" ]; then
    log "reusing cached parallax ${version}"
    return
  fi

  mkdir -p "${prefix}/bin"
  curl -fsSL \
    "https://github.com/sarus-suite/parallax/releases/download/v${version}/parallax-v${version}-ubuntu-24.04-amd64" \
    -o "${prefix}/bin/parallax"
  curl -fsSL \
    "https://github.com/sarus-suite/parallax/releases/download/v${version}/parallax-mount-program-v${version}.sh" \
    -o "${prefix}/bin/parallax-mount-program"
  chmod 0755 "${prefix}/bin/parallax" "${prefix}/bin/parallax-mount-program"
}
